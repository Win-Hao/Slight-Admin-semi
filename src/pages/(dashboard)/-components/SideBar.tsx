...
const SideBar = ({isCollapseButton}: { isCollapseButton: boolean }) => {
    ...
    const createNavMenu = (items: MenuItemType[]) => {
        return items.map((item: MenuItemType) => {
            ...
            if (item.items) {
                return (<Nav.Sub key={itemKey} itemKey={item.itemKey} text={text} icon={icon}>
                    {createNavMenu(item.items)}
                </Nav.Sub>)
            } else {
                return (
                    <Nav.Item key={itemKey} icon={icon} itemKey={itemKey} text={text} onClick={({itemKey}) => {
    ...
    return (
        <Nav
            ...
            header={{
                ...
                text: <span className='text-semi-color-primary !font-bold cursor-pointer'
                            onClick={() => {
                                navHandler('/dashboard/workbench')
                            }}>Slight Admin</span>,
            }}
